...
本轮**不重建任何数据**——678 GB 原始建库产物、packed 库、norm stats 全部原样不动。所以要保证的其实是：**将来再跑建库链时，代码的行为和今天逐位相同**。用三层证明叠加，一层比一层兜底：

1. **源码同一性（机器判定，不靠测试猜）**：四个共用文件复制进建库目录时，用 diff 逐字节核对——三个文件必须零差异，主文件的差异必须恰好是 3 行 import 指向替换，函数体一个字符不许动。这一过，「计算逻辑没变」就是源码级证明。
2. **依赖闭合（隔离与一致互相成全）**：后续几个提交会删掉、瘦身训练侧的那批共用文件——如果建库脚本还绑在训练侧代码上，一致性当场就破。隔离后用双向检查钉死：文本搜索保证建库文件不再引用训练侧任何模块；再在全新 Python 进程里实际加载建库入口，确认没有任何训练侧模块被连带载入（这一条抓得住文本搜索看不见的间接依赖）。
3. **行为抽查（兜「万一看漏」）**：用建库链自带的零容差复算校验，在本机同显卡型号建出的真值库上抽 32 条，从原始 H5 重算特征与存盘产物逐位比对；另在同一进程里让旧代码与新副本吃同一批真实帧、同一个视觉编码器，输出全键逐位对拍。若抽查失败，先用改动前的旧代码跑同一批样本做对照——对照也挂说明是环境漂移（驱动/库版本），与重构无关；对照过而副本挂才是拷贝出了错。

另加一条**防退化哨兵**：测试里固化建库域主文件的哈希值，防止将来有人好心把训练侧的改动「同步」回建库域副本，让冻结悄悄失效。

两个如实声明的边界：抽查**不能**对 GL 集群建出的正式库跑（不同显卡型号重算做不到逐位一致，仓库已有实证；且校验命令的收尾会改写库的元数据文件——那是黄金基线指纹的一部分，跑一次基线就作废，故只允许调用其中的纯抽查函数）；GL 侧也不重建重验——本轮证到的是「代码逐字节同一 + 本机同架构复算逐位一致」，「GL 上重建能得到同样结果」是由此推出的结论而非实测，计划盲区表里已如实登记。

### 6.2 「G3 对拍 G0 就能证明训练语义没变」怎么成立

G0 是重构前训练语义的**黄金基线**：在受控确定性环境下真实训练 1000 步，把逐步的 loss 等五个标量（精确到浮点位）、12 个时点的完整模型参数与优化器状态摘要、14 个时点喂进模型的每个 batch 的逐键指纹、全部样本抽取顺序，全部固化进了 git。它能跨期充当「位级对比」的一侧，靠三个已经验证过的前提：

1. **复跑零抖动**：同配置重跑两次结果逐位一样，这在四档确定性实验里证过，其中最严的一档是「清空编译缓存、独立重编译两次仍逐位一致」——这意味着 G3 虽然代码变了、必然现场重编译，对拍依然成立。G0 自己两轮千步互证逐位相同，之后 G1（dtype 修复后）、G2（packed IO 后）都复现出同一个哈希，等于这条管线的复跑噪声被四份独立证据钉死为严格零。
2. **环境没漂**：起跑前必须过环境指纹检查（依赖锁文件、库版本、显卡驱动、权重与数据资产的哈希等数十项逐一比对），任何一项不符基线即作废。要注意它**不含代码哈希**——所以指纹通过只说明「还有资格引用 G0」，绝不能当「代码没改坏」的证据，后者只能由对拍本身回答。
3. **对拍是全方位的位级比较**：G3 与 G0 同种子、同数据、同批大小跑 1000 步后，四个维度全部要求逐位相同——每步五个标量的浮点位、参数/优化器状态摘要、喂进模型的 batch 内容指纹、样本抽取顺序。只要重构在任何一跳改了任何一个字节（数据装配、变换、模型前向、梯度、优化器更新），差异会在标量或摘要里位级暴露，藏不住。最后把整张标量表压成一个哈希值，与 G0/G1/G2 共同的那个值比对，一行收官。

一句话：**前提 1 保证「差异不可能来自重跑噪声」，前提 2 保证「差异不可能来自环境」，于是 G3 若逐位等于 G0，唯一的解释就是七个提交合起来没有改变训练语义；若不等，唯一的解释就是改了**——再用分段闸门（5 步烟测、单步梯度对拍等）从便宜到贵定位是哪一段引入的。
