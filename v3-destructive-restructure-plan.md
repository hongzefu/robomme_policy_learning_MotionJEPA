...
**G3 之后另有追加的统一入口工作**（2026-08-29 用户显式批准；原编号 commitV3.15，2026-08-30 改定 commitV5.0）：`scripts/training/main.py` 统一入口与入口等效性验证。它不在上表七刀序列内、不改训练链任何文件、不影响 G3 判据，权威文档为仓库根目录 `v5.0-train-entry-restructure-plan.md`（文末第三部分已缩为指针；2026-08-30 二次改向：不再新增 main.py，改为 train.py 本体单跑收敛 + bench 拆分 g0/util + 双重对拍，详见该文件）。
...
# 第三部分（已拆出）：训练入口工作 —— 见 `v5.0-train-entry-restructure-plan.md`

> 本部分正文已于 2026-08-30 拆出为仓库根目录独立计划，以该文件为唯一权威，本处仅留指针。commit 编号由原批准的 V3.15 改定 **commitV5.0**（V4.x 序列已落地，避免 git log 编号倒序）。方向沿革（同日三版，均见新文件 git 历史）：①本部分原稿 = 新增 `main.py` 统一入口（双跑照抄 + 同分支对拍）→ ②单跑薄封装 + main 分支对拍（`v5.0-unified-entry-plan.md` 初稿）→ ③**现行方案**：不再新增 main.py，`train.py` 本体破坏性改单跑（≡ prod 语义）、删 `prod_train_once.py` 由 sbatch 直调、bench/ 拆分为 g0/ 与 util/、G0 完整重锚 + 上游 main（`ecf086c`）官方入口真跑对拍。原第三部分全文可在本文件的 git 历史查阅。第一、二部分（V3.8–V3.14 与 G3）内容与权威性不受影响。
