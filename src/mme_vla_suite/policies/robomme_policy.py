...
class RoboMMEInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        inputs = {
            ...
            "motion_emb": data.get("motion_emb", None),   # (motion.budget, 768)
            "motion_pos": data.get("motion_pos", None),   # (motion.budget, 256)
            "motion_mask": data.get("motion_mask", None), # (motion.budget,)
            "mem_order": data.get("mem_order", None),     # (帧路 budget + motion.budget,) int32 置换
        }
