...
`metadata.contract.input_comparison` 当前固定为 `host_numeric_exact_motion_none_v1`，另完整记录 `equivalent_motion_none_keys` 四键白名单；judge 使用 `numeric_comparison_tree()` 生成数值比较视图，只在支持的数值叶上忽略原 dtype 描述、字节数和 raw SHA 的差异。原始全部字段仍在 `raw_all/transformed_all/inputs_all` 留证，除下一段获准的 None 白名单外，shape、字段存在性、容器顺序和有限性均保持严格。缺失数值摘要、旧 schema 或旧比较契约不能回退通过，正式取证须使用当前工具。成功判定行含 `INPUT_EQ=PASS ... comparison=host_numeric_exact_motion_none_v1`，不能用三样本诊断冒充正式两库全量结果。

**motion 四键只放行已获准的缺失/None等价。** 真实既有40集库索引0/31/32的raw/transforms共同字段数值相同，B侧独有 `motion_emb`、`motion_pos`、`motion_mask`、`mem_order` 且全为None，见[三样本证据](docs/training-doc/orig80k-schema-0925/result.md)。用户看过结果后明确「允许这四键缺失与 None 等价」。因此比较投影仅在这些键缺失或值严格为 `None` 时排除它们，原始证据与真正训练输入均不改；出现任意非None即失败，包括零数组和False，其他新增/缺失键仍失败。训练侧 `entry_equiv.py` 的五标量 `float.hex()` 和参数、EMA、优化器状态及step摘要仍要求bitwise相同。

后续完整输入判据：两库分别核对全量样本身份及 sampler 的前两个完整 epoch 顺序（含 `drop_last` 与重建 iterator 边界）；每集选执行首尾、demo/exec 交界及历史不足/达到/超过 32 帧的合法样本，比较原始样本和 transforms 后的全部模型输入；另比较正式 b64/workers4 的前 100 个 batch，并覆盖 epoch 换轮前后各两批。仅日志路径、运行身份、不进入模型的历史废弃键，以及上述获准的四个 motion 键缺失/严格 `None` 情形可列明排除，不得忽略其他模型输入键。主机数值按上述无损口径精确相同，shape、白名单外的 schema 和顺序仍严格一致，dtype/raw SHA 差异单列留证；总判使用 `INPUT_EQ`，失败打印首个样本身份与键名。有限输入取证须明确样本与 batch 范围，不把小范围结果冒称此处全量判据通过。

### 2. 执行顺序：环境与预算 → 构建冒烟 → 两个正式库 → 输入与训练对拍 → 测速 → 正式 80k

用户已恢复建库范围，以下流程从已完成的 P0 及待核实的来源、预算闸门继续推进。必须先完成第 0 步，再进入第 1 步构建冒烟和正式库；训练侧仍按第 2–4 步逐关验收，P0 或工具测试通过不能替代任何后续闸门。motion四键的有限豁免已有裁决，正式输入仍须逐项核对，不将白名单扩大到非None或其他键。
...
3. **输入量具**：`scripts/training/tests/check_orig80k_inputs.py` 在 A/B 环境分别 import 该侧 Dataset、transforms 和真实 loader，按 §1.2 样本与 batch 计划取摘要，禁止 A 导入当前仓库参考实现。本次按最新决定补充无损精确数值摘要及 `MOTION_NONE_KEYS` 四键缺失/严格 `None` 等价判定；原始 `raw_all/transformed_all/inputs_all` 保留全部字段和 dtype/raw SHA、shape、有限值记录。当前协议为 `host_numeric_exact_motion_none_v1`，`metadata.contract.equivalent_motion_none_keys` 必须完整登记四键；原始 dtype 不同本身不再阻断，任何非 `None` 的 motion 值、其他新增/缺失键或精确数值差异仍阻断，训练标量和状态仍逐位比较。现有 `dump_fixture_samples.py` 的 `DTYPE_DUMP_IMPL=refnpy|packed`、`DTYPE_DUMP_MODE=both`、`DTYPE_DUMP_ARRAYS=0` 可作为当前 source→packed 的额外检查，正式取证不设 `DTYPE_DUMP_LIMIT`；其现有 batch 固定为 8、单进程 collate，不能冒称正式 b64/workers4 检查。真实 sampler/collate 量具与有限输入范围的结果应明确区分；全量及训练对拍仍须在数据和相应输入闸门满足后进行。
...
8. **最小验证**：工具测试位于 `scripts/training/tests/` 与 `scripts/dataset/`，单测尽量5分钟内。P0档案记录首轮合测271项；精确数值口径曾通过123项针对测试，三样本CPU字段取证已完成。加入用户批准的四None键白名单后，输入工具与训练判定器合测128项通过、12.32秒，含只放行缺失/None以及非None拒绝。以上均不能替代正式两库完整输入取证、P1及100步训练对拍。
...
**范围与决定**：公开 16×100 全集、四 counting×100 子集、modul、原版超参、counting 重算 norm_stats、两组名称与 GPU 分配沿用。此前「本轮只实现和验证工具，暂不建库」是已被后续覆盖的阶段性决定；当前依据最新原话「恢复计划中的建库，严格按前置闸门推进」继续来源、路径与实际预算检查、构建冒烟及两个正式库。训练仍须满足所有对应前置，不因恢复建库而跳过输入、对拍、perf 和完成验收。主机 dtype 可不同，但精确数值与训练标量/状态 bitwise 要求不变。此前按「先保留严格判据并取证，结果出来后再决定」取得的三样本原始 schema 失败事实保留；用户随后明确「允许这四键缺失与 None 等价」，当前仅对 `motion_emb`、`motion_pos`、`motion_mask`、`mem_order` 缺失或值严格为 `None` 的比较投影放行，原始 ALL 字段和实际训练输入不改，任何非 `None`、其他键或数值差异仍失败。每阶段重测磁盘及输出冲突，已有名称、数据口径等决定无需重复询问。
