...
> 创建日期 2026-09-17（America/New_York）。计划首次入库 commit `5b2e7ba`，之后只有 docs commit 修订本文件。**「改前 / A 侧」= 起跑时主副本 `v2-motionmem` 的 clean HEAD（记作 `A_HEAD`），其源码与 `e4dc733` 逐字节相同**（`e4dc733` 之后全是 docs commit）；`A_HEAD` 的具体 SHA 在第 4 节第 1 步记录进 `lock_sha256.txt` 与 `launch.md`，本文件不钉死它，否则每改一次计划就失效。
...
**改前和改后是磁盘上两个不同的目录、两个不同的分支，同时存在**——不是同一份代码先跑一次再改了再跑一次，而是改前那份从头到尾不动，改后那份在另一个目录里改、在另一个目录里跑，两份并排放着互相对照：

| | 改前副本 | 改后副本 |
|---|---|---|
| 目录 | `/scratch/hongze/robomme_policy_learning_MotionJEPA`（现在这份，即主副本） | `/scratch/hongze/robomme_policy_learning_MotionJEPA-temp`（第 2 步新建，即开发副本） |
| 分支 | `v2-motionmem`，HEAD 记作 `A_HEAD` | `v2-motionmem-collate-shm`（第 2 步从 `A_HEAD` 切出） |
| 源码 | 与现在完全相同，第 3 步起锁只读，全程不加 commit | 加上那 20 行改动，共 3 个 commit：C1 代码、C2 起跑留档、C3 结果留档 |
| 数据 `v1-store` | 实体目录 | 指向改前副本 `v1-store` 的 symlink，同一份数据、不复制 |
| `.venv` | 现有的 | 自己单独装一份 |
| 角色 | 只当对照，跑「改前」那一侧 | 改代码、跑「改后」那一侧、写留档 |

**九步，每步一句话**（命令、参数、判定行原文见第二部分对应小节）：

| # | 在哪个副本 | 做什么 | 为什么 | 产出 commit | 通过条件 |
|---|---|---|---|---|---|
| 1 | 改前副本 | 确认起点干净：HEAD 与 `e4dc733` 源码一致、没有别人的在途改动被卷进来、8 卡空闲 | 后面所有「改前」数字都以这个 HEAD 为准 | 无 | 不干净就停下来问你 |
| 2 | 改前副本 → 新建改后副本 | 从改前副本切出新分支，放到 `-temp` 目录，`v1-store` 用 symlink 共享，装自己的 `.venv` | 改动与对照物理隔离，数据不复制第二份 | 无 | 开发副本 HEAD 等于 `A_HEAD` |
| 3 | 改前副本 | 把改前副本的源码目录设成只读，记下三份关键文件的 sha256 | 保证「改前」在整个对比期间一个字节都没变 | 无 | 目录权限变成只读 |
| 4 | 改后副本 | 落那 20 行改动，加两个小测试；在 CPU 上跑「改前 vs 改后」逐键字节对拍 | 先不碰 GPU 就证明喂进模型的数据完全一样（第 18 条第一块） | **C1 `commitV9.8`** | 对拍 0 处不匹配，测试全绿；否则不提交 |
| 5 | 改后副本 | 写起跑留档 | 记下两侧 commit、命令、配置，满足 preflight 的 clean HEAD 要求 | **C2 `docs:`** | 开发副本工作区干净 |
| 6 | 两个副本各起一次，8 卡独占 | 先从改前副本起 300 步，跑完再从改后副本起 300 步，同配置，比步时和 GPU 利用率 | 这是你要的速度对比；改前那档还要复现上次的 1.82 s 作校验 | 无 | 两档都正常退出；得到 old / new 步时与提速倍数 |
| 7 | 两个副本各起，8 卡独占 | 改前副本起两次、改后副本起一次，各 100 步、确定性模式，逐步比 loss / 梯度范数 / 参数摘要 | 证明训练本身没被改变（第 18 条第二块）；改前跑两次是为了知道「同代码重跑」本身是否逐位一致 | 无 | 改前 vs 改后与改前 vs 改前一样逐位一致 |
| 8 | 改后副本 | 写结果留档，收原始记录，删临时 run 产物 | 第 12、17 条要求；结果不留在 tmux 里 | **C3 `docs:`** | 分支上依次是 C1、C2、C3 |
| 9 | 改前副本 + 你 | 问你三件事：推不推新分支到远端、合不合并回 `v2-motionmem`、解不解锁改前副本 | 新分支没有 upstream、合并与解锁都是你的决定 | 无 | 你答复前改前副本保持只读、分支留在本地 |

**三条规则**：
- 改前侧永远是改前副本的 `A_HEAD`，改后侧永远是改后副本的 C2（C2 和 C1 源码相同，只多一份起跑留档）；两份副本全程并存，直到第 9 步你决定合并后才删改后副本。
...
- **A 侧（改前）**：主副本 `/scratch/hongze/robomme_policy_learning_MotionJEPA`，HEAD `A_HEAD`（源码已锁只读，只读不妨碍运行）；run 名 `bench-collate-shm-old`。
...
git rev-parse HEAD                # 记为 A_HEAD；git diff --stat e4dc733 HEAD -- src scripts packages 须为空
