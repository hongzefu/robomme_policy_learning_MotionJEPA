...
**执行状态（2026-09-17 更新）**：用户本轮明确指示「开始执行 有问题问用户越早越好 结束后报告 问用户是否合并」，实施与验收已完成，结果见[正式留档](docs/training-doc/bench-collate-shm-8gpu/result.md)。起点为 `7a681e2cf3f21bdab3f0b27ea44d19b0adfd0ee7`；C0/C1/C2 分别为 `82d73f4`、`3868cc9`、`24f8cc9`，结果归档为独立 C3。20 批输入与 100 步三侧训练对拍全部逐位通过；八卡步时 1.773→0.973 秒，提速 1.82 倍，利用率均值 53.32%→97.87%。临时 run 已清理，两个副本保留；推送、合并和删除副本等待用户决定。

执行时用户确认三项前置修正：主副本按实际干净状态验收 PASS，CPU 自检改为真实配置解析及训练前入口守卫，在本地 `.git/info/exclude` 加精确 `/v1-store` 以忽略已授权的存储链接。完整状态摘要比估时慢，用户进一步明确「继续完成原定验证，允许延长占用」。实际 GPU 验证总耗时 2 小时 14 分 13 秒，判据和摘要数量均未放宽。

历史说明：计划编写阶段曾按用户「不要开始」停在 `96b86b5` 的只读核验，彼时未建副本、未改权限；该暂停已由本轮新的执行指令替代。原有其他 worktree 和 tmux 会话均未清理。
