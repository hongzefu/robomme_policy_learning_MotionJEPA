...
**最终实况（2026-09-17 收尾）**：训练于 `2026-09-16T20:53:31Z` 以 `EXIT_CODE=0` 结束，60k 步全程耗时 39h05m04s，12 个 checkpoint 全部落盘。最终 `59999` 在 CPU 上成功读取，参数树 61/61 精确匹配；主副本已解除写保护并快进同步开发副本的三个文档提交。训练验收留档 `9f7f423` 推送后，开发副本已按用户批准删除，主数据及全部 checkpoint 验收未变。最终结果、统计复现与清理验收以 [result.md](result.md) 为准；本文件的起跑命令及原始版本信息继续作为历史记录。
