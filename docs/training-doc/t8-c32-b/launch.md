...
此记录为已授权计划的正式1000更新轨迹准备；只有400ep完整输入验收与对应100步排错通过后才启动。用户原话：“开始实现 有问题越早问用户越好 一口气全做完！”，以及“注意你只能用4个gpu”。本轨迹使用物理GPU 4,5，同profile三条始终同卡对，fsdp 2。run_name为用户已确认的 `t8-c32-b`，启动前断言记录与checkpoint根均不存在，禁止overwrite/resume。
...
下列命令体放入detached tmux `t8-c32-b`，外层 `set -o pipefail` + `tee v1-store/logs/t8-c32-b.log`，结束记录UTC时间与 `EXIT_CODE=${PIPESTATUS[0]}`。同profile按A1→A2→B运行；A1/A2完整重复性先通过再起B。阶段5十二条轨迹期间不再提交代码或文档。
...
export BENCH_RECORD_DIR=/scratch/hongze/robomme_policy_learning_MotionJEPA/v1-store/bench/8x8/t8-c32-b
...
export MMEVLA_JAX_CACHE_DIR="$V1_STORE/cache/jax/t8-c32-b"
...
test ! -e "$V1_STORE/train-runs/t8-c32-b"
...
uv run --no-sync python scripts/training/g0/bench_train_steps.py mme_vla_suite --exp-name t8-c32-b \
 ...
 --checkpoint-base-dir "$V1_STORE/train-runs/t8-c32-b" --batch-size 8 --num-workers 4 --num-train-steps 1000 --log-interval 1 --save-interval 1 --seed 42 --fsdp-devices 2 \
 --dataset-path "/scratch/hongze/robomme_policy_learning_MotionJEPA/v1-store/datasets/4task-motion-400ep/framesamp" --weight-loader.params-path "$V1_STORE/models/openpi-assets/checkpoints/pi05_base/params" \
...
JAX_PLATFORMS=cpu uv run --no-sync python scripts/training/g0/check_baseline_env.py manifest "$BENCH_RECORD_DIR"
JAX_PLATFORMS=cpu uv run --no-sync python scripts/training/g0/check_baseline_env.py check --base "/scratch/hongze/robomme_policy_learning_MotionJEPA/v1-store/bench/8x8/t8-c32-a1" --record-dir "$BENCH_RECORD_DIR" --steps 1000 --batch-size 8 --dataset "/scratch/hongze/robomme_policy_learning_MotionJEPA/v1-store/datasets/4task-motion-400ep/framesamp"
printf 'TRAJECTORY_DONE run=t8-c32-b steps=1000\n'

```

每次更新取五标量hex，完整TrainState取11个state_step `0,2,3,25,50,100,200,400,600,800,1000`，177叶；11份batch含None键与样本索引，主进程索引前8000项逐位比较且记录数至少8072。有限性、参数活性、真实norm_stats与导入来源同时通过，才接受对应profile的GATE_8X8。本条不额外保存大权重，完整参数、优化器和EMA由逐叶摘要取证。
