...
生产扩容和工具已实现；输入、初态、旧两档完整训练回归、最终功能门及100步保存加载全部通过。新档packed对照仍在推进，容量及独立测速尚未完成，当前不宣称整套验收结束。
...
接续完成新参考/packed20+1双侧比较。8卡20步只验容量，1000步独立测速取0..99预热、100..899稳态、900..999尾段与真实保存；不启用profiler，不逐步同步，GPU500ms采样，主机及并发IO独立记录。80k用时只标估计；满足上方已获条件授权后执行V10和正式起跑，否则报告未满足项。
