...
生产扩容和工具已实现；输入、初态、旧8×64的完整训练回归与新2048正式功能门通过。旧32×16与新档训练对拍仍在推进，保存加载、容量及独立测速尚未完成，当前不宣称整套验收结束。
...
接续完成旧32×16与新参考/packed20+1、原数组到checkpoint动作对拍。8卡20步只验容量，1000步独立测速取0..99预热、100..899稳态、900..999尾段与真实保存；不启用profiler，不逐步同步，GPU500ms采样，主机及并发IO独立记录。80k用时只标估计；满足上方已获条件授权后执行V10和正式起跑，否则报告未满足项。
