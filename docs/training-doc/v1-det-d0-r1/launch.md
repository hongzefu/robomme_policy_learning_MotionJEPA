# v1-det-d0-r1（P2 确定性预备实验，档位 D0）

- **目的**：现状档：每轮删编译缓存、不注入 XLA flags——「字面现状」存档 + 自身重跑噪声底（预期 FAIL，非判据基线）。四档定义与判定见 `v1-gradient-baseline.md` P2 节；本轮为该档第 1 轮。
...
- **命令**：`STEPS=100 SAVE_INTERVAL=50 WARMUP_STEPS=50 WORKERS=4 RUN_TAG=v1-det-d0-r1 EXP_NAME=v1-det-d0 KEEP_JAX_CACHE=0 XLA_FLAGS="" bash scripts/smoke-local/run_2gpu_epoch_bench.sh`（由 P2 驱动脚本顺序调用，全程 tmux + tee）
