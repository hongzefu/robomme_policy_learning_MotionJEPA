...
完整 dump 不设 limit，保留全部键的 dtype/shape/raw/canonical 摘要，200 批。max_frames=8，3200 样本。原始输入逐帧与 pkl 身份全部校验；产物暂不表示候选版本等价，须等 CAND 对拍。

下列命令体放入 detached tmux `m2048-dump-base-8x64`；外层用 `set -o pipefail`、`tee v1-store/logs/m2048-dump-base-8x64.log`，结束记录 `EXIT_CODE=`。本档会话只有该完整名称。原始产物保留 v1-store/bench/m2048/，清洗日志与摘要随后归档 records/；checkpoint 不进 Git。
...
export DTYPE_DUMP_DIR="$V1_STORE/bench/m2048/m2048-dump-base-8x64"
