...
会话名为 `vail-eval-local-20260918T0400Z`，在 detached tmux 中执行以下命令。准备会话另有 mv-eval-uv-0917、mv-eval-ckpt-0917；只记录和清理自己的进程，不释放用户交互分配。
...
set -o pipefail
CUDA_VISIBLE_DEVICES=1 bash scripts/evaluation/run.sh vail-eval-local-20260918T0400Z "$REPO/v1-store/models/robomme-vla-modul-60k-v1/59999" RouteStick 1 7 2>&1 | tee "v1-store/logs/policy-eval/vail-eval-local-20260918T0400Z.log"
echo "EXIT_CODE=$?"
```

结果与完整视频位于 v1-store/evaluation/vail-eval-local-20260918T0400Z/perceptual-framesamp-modul-8frame-8x8/ckpt59999/seed7/。留档保存轻量原始日志和 JSON，不提交权重与视频。收官分别报告任务结果和链路验收。
