...
AWS A100-SXM4-80GB，物理GPU `6,7`，存储为 `/dev/md0` XFS本地NVMe RAID。开始 `2026-09-14T04:41:00Z`，结束 `2026-09-14T05:52:33Z`，墙钟1小时11分33秒；11份状态摘要累计2503.0秒。
...
| 慢步分层 | n=0，均值无样本 |
| 其他步分层 | n=6322，均值96.34% |
| 整卡显存峰值（含预分配） | GPU6: 78641 MiB；GPU7: 78641 MiB |

慢步阈值为该run稳态步时中位数的1.5倍（2.5298秒），中位数只用于分层。完整计算方法和排除步列表保存在[性能记录](records/performance_stratified.json)，原始采样保留于records；未用单一利用率断言无瓶颈。
