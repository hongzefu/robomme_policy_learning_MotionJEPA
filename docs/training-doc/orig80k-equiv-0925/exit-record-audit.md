...
新CPU输入包装已修复并完成两轮独立纯shell验证，但**CPU输入采集尚未启动**。遵照用户“有问题立刻问用户 不要自己决策”的要求，发现历史边界后曾暂停并询问处置。用户随后依次批准P1/100步对拍关闭W&B、perf只读磁盘采样，最后明确「补记限制，继续 CPU 输入取证」，范围与时序见文末。三项均已决定，允许从本轮提交后的clean INPUT_HEAD继续；批准不把历史观测缺项写成已经补齐。
...
审计发现问题时曾询问用户：补充上述历史说明后，沿用数据内容和20步保存恢复子任务的成功证据继续CPU输入取证；或先制定额外复验方案。当时暂停了CPU起跑，未默认选择，也未启动CPU采集、P1、100步、perf或80k。随后获得以下三项明确决定，原审计取证锚点与事实不因事后批准而改变。
...
用户随后明确批准「允许仅对拍关闭 W&B」及「补充 0.5 秒只读采样」。前者仅限P1/100步A/B对拍关闭W&B，保留本地完整标量和状态；20步、300步perf及80k继续开启。后者是在perf期间只读采本轮checkpoint文件分配字节和`/scratch`可用字节，记录0.5秒采样观察到的峰值并保留保守预算余量。批准不等于实现、验证或实测已完成，也不补造历史保存峰值；本审计的固定锚点、两轮纯shell测试事实及原始records保持不变。

在上述两项批准之后，用户明确选择「补记限制，继续 CPU 输入取证」。因此保留数据、训练和恢复子任务及产物PASS，如实补记最外层退出观测缺项；不改写历史记录或自行追加历史重跑。CPU尚未运行，允许按原命令和判据从本轮提交后的clean INPUT_HEAD启动；后续训练和perf仍须各自前置验收。主机dtype许可、四键缺失/None许可和训练标量/状态逐位要求均不改变。
