...
| 目录 | 内容 | 判定 |
|---|---|---|
| [`orig80k-schema-0925/`](orig80k-schema-0925/launch.md) | 既有40集库三样本CPU输入字段取证，保留dtype及精确数值摘要 | 预建；motion缺键/None仍严格判定，不替代正式两库验收 |
| [`orig80k-env-0925/`](orig80k-env-0925/result.md) | 上游独立uv环境P0、源码恢复和模块来源验证 | P0通过，208项依赖版本一致、退出0；最新决定恢复建库并逐关验收，motion仍严格取证待裁决，训练须全部前置闸门 |
