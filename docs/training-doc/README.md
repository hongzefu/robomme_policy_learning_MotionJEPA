...
| [`binfilldemo-nomotion50k-gl/`](binfilldemo-nomotion50k-gl/result.md) | **BinFill 补 demo 前缀重测的主结果页**（三个 run 共用对照表与起跑留档）。上面三条 run 在 BinFill 上的反常退化，根因是评测输入缺了训练时一直存在的 demo 前缀段；同一批 142 条上补齐后 512 档 30.3%→**51.4%**，且三者排序从颠倒恢复成与其它三任务一致。含「这些数字不能怎么用」与盲区清单 |
