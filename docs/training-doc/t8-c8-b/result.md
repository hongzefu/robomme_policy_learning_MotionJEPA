...
AWS A100-SXM4-80GB，物理GPU `4,5`，存储为 `/dev/md0` XFS本地NVMe RAID。开始 `2026-09-14T10:12:56Z`，结束 `2026-09-14T11:16:11Z`，墙钟1小时3分15秒；11份状态摘要累计2133.6秒。
...
| 稳态步时均值 | 1.5577秒 |
| 吞吐 | 5.1357 samples/s |
| GPU util均值／0%采样占比 | 96.09%／0.00% |
| 慢步分层 | n=0，均值无样本 |
| 其他步分层 | n=5838，均值96.09% |
| 整卡显存峰值（含预分配） | GPU4: 78639 MiB；GPU5: 78639 MiB |

慢步阈值为该run稳态步时中位数的1.5倍（2.3354秒），中位数只用于分层。完整计算方法和排除步列表保存在[性能记录](records/performance_stratified.json)，原始采样保留于records；未用单一利用率断言无瓶颈。

## 产物与后续

本条保留1000更新后的EMA checkpoint：`/scratch/hongze/robomme_policy_learning_MotionJEPA/v1-store/train-runs/t8-c8-b/mme_vla_suite/t8-c8-b/999`，目录编号999。权重不进入Git，后续推理只使用该产物。
