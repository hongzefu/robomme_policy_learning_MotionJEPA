...
下列命令体放入 detached tmux `m2048-b20-base-32x16`；外层用 `set -o pipefail`、`tee v1-store/logs/m2048-b20-base-32x16.log`，结束记录 `EXIT_CODE=`。本档会话只有该完整名称。原始产物保留 v1-store/bench/m2048/，清洗日志与摘要随后归档 records/；checkpoint 不进 Git。
...
for M2048_STEPS in 20 1; do
  M2048_RUN="m2048-b20-base-32x16"
  if [ "$M2048_STEPS" = 1 ]; then M2048_RUN="m2048-b20-base-32x16-step1"; fi
