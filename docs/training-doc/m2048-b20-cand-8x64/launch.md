...
会话全名m2048-b20-cand-8x64。以下命令体经detached tmux运行，外层pipefail与tee写v1-store/logs/m2048-b20-cand-8x64.log，正常或失败均追加EXIT_CODE。每档结束先核会话退出和GPU释放；任何旧档数值失配按计划仅撤销生产提交并报告，harness错误另行定位。主run应有20行标量、20状态行（更新0及2..20）、232个索引/29批；补跑有0/1两个状态、80个索引/10批。当前仅建启动档案，尚未运行。
...
for M2048_STEPS in 20 1; do
  M2048_RUN="m2048-b20-cand-8x64"
  if [ "$M2048_STEPS" = 1 ]; then M2048_RUN="m2048-b20-cand-8x64-step1"; fi
  M2048_REFERENCE="m2048-b20-base-8x64"
  if [ "$M2048_STEPS" = 1 ]; then M2048_REFERENCE="m2048-b20-base-8x64-step1"; fi
