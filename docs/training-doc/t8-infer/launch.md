...
本记录承接[完整计划](../../../0908-8frame-8x8-training-plan.md)，使用已经通过四组1000步逐位训练验收的两份候选EMA checkpoint。用户要求“一口气全做完”，限定四张GPU，并确认“让评估脚本读取 checkpoint 保存的配置，自动算出正确长度”；C8前缀应为1088，M8应为1184。新代码固定在CAND `c08ec2060a544af1869c1e24f755e536150569ca`，之后只有文档提交；各次运行的实际clean HEAD和UTC时间由外层日志记录。
