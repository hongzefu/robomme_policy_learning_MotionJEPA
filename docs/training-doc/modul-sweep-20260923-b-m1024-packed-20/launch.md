# 1024 packed 20步真实输入、五标量与201叶取证

当前为预建档案，尚未启动。本档由[顺序实施总档案](../modul-sweep-20260923-b/launch.md)管理；不得把预建时间当作实际起跑时间。
...
全局batch128、worker16、FSDP8、seed42、20步；history文件为 `perceptual-framesamp-modul-16frame-8x8.yaml`。验证仅由启动覆盖缩短步数并关闭W&B；正确性取证使用log1/save1与确定性XLA_FLAGS，容量及测速清除XLA_FLAGS。
