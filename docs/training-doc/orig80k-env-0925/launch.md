...
P0完成后先收到的决定原话为：「允许主机 dtype 不同，但要求数值一致且训练标量/状态逐位一致」、「本轮只实现和验证工具，暂不建库」及motion四键「先保留严格判据并取证，结果出来后再决定」。该阶段限定于工具及有限非训练输入取证：原始dtype/raw SHA继续保存，主机数值按根计划§1.2无损精确比较，signed zero保持严格；motion缺键/None仍失败，训练标量与状态bitwise判据不变。这些是事后范围记录，不改变P0命令、启动提交或当时的授权事实。

用户随后明确「恢复计划中的建库，严格按前置闸门推进」，覆盖上述暂停建库决定。当前可按来源pin、输出路径、实际磁盘预算和构建冒烟的次序恢复建库；最新只读复核可用`1792014577664 B`、使用率77%，8GPU全空，两个新库及counting硬链接目录不存在。该快照不是新容量门槛或预算通过结论；训练仍须全部对应闸门，motion四键仍先严格取证、结果出来后交用户裁决。
