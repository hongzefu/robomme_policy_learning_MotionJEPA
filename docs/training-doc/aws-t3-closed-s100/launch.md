# aws-t3-closed-s100（T3 closed 态 100 步）launch
...
- **目的**：T3 关闭态对照 run：与 aws-t3-open-s100 同 seed / 同库 / 同口径，只差 YAML 的 motion.enabled；提供 t3verifyinit（177 叶命中 reference）、T3_SMOKE 的 closed 侧、t3trace 的公共 12 叶与 index 前缀、t3phase 的 closed 最终 ckpt。
- **HEAD**：`8093ebda23ec566533067e319bab506baaf80de5`（commitV6.12，clean）。history config **固定** `perceptual-framesamp-context.yaml`（红线 16，不手改开关）。
...
```bash
STEPS=100 SAVE_INTERVAL=25 EXTRA_DIGEST_STEPS=99 WORKERS=4 WARMUP_STEPS=20 HISTORY_CONFIG=perceptual-framesamp-context.yaml \
BENCH_SAVE_FINAL_CKPT=1 BENCH_FINAL_STEP=99 EXP_NAME=aws-t3-closed-s100 RUN_TAG=aws-t3-closed-s100 BENCH_GPUS=4,5 \
...
bash scripts/training/g0/run_2gpu_epoch_bench.sh 2>&1 | tee v1-store/logs/aws-t3-closed-s100-driver.log
```

- **GPU**：CUDA 4,5。**起跑前**：`t3common`（GPU2,3，`--fsdp 2`）已冻结 reference `v1-store/reports/motion/t3_common_init_reference.json`（`T3_COMMON_INIT=PASS`）。
