# m2048-dump-packed 起跑口径

按用户「一路实现到测速结束报告用户为止」及「保持原计划，完整取证（推荐）」执行。该项只读1600ep库：framesamp-8x8；history配置perceptual-framesamp-modul-32frame-8x8.yaml，输入实现packed。不创建派生库，不抽SigLIP，不改norm_stats。CPU运行，CUDA_VISIBLE_DEVICES为空，JAX_PLATFORMS=cpu。工作区须为生产与验证工具提交后的同一clean CAND，实际完整HEAD与UTC由启动日志记录。
...
唯一会话名m2048-dump-packed，以下命令体放detached tmux；外层pipefail+tee写v1-store/logs/m2048-dump-packed.log，结束追加EXIT_CODE。每份日志独立流式监听，当前仅准备入口。
...
export DTYPE_DUMP_DIR="$V1_STORE/bench/m2048/m2048-dump-packed"
test ! -e "$DTYPE_DUMP_DIR"
uv run --no-sync python scripts/training/tests/dump_fixture_samples.py mme_vla_suite --exp-name m2048-dump-packed \
