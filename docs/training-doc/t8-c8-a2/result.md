...
AWS A100-SXM4-80GB，物理GPU `4,5`，存储为 `/dev/md0` XFS本地NVMe RAID。开始 `2026-09-14T09:07:50Z`，结束 `2026-09-14T10:11:21Z`，墙钟1小时3分31秒；11份状态摘要累计2159.9秒。
...
| 稳态步时均值 | 1.5555秒 |
| 吞吐 | 5.1429 samples/s |
| GPU util均值／0%采样占比 | 96.43%／0.00% |
| 慢步分层 | n=0，均值无样本 |
| 其他步分层 | n=5828，均值96.43% |
| 整卡显存峰值（含预分配） | GPU4: 78639 MiB；GPU5: 78639 MiB |

慢步阈值为该run稳态步时中位数的1.5倍（2.3367秒），中位数只用于分层。完整计算方法和排除步列表保存在[性能记录](records/performance_stratified.json)，原始采样保留于records；未用单一利用率断言无瓶颈。
