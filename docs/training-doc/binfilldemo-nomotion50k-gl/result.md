# 五模型完整结果：50000 与最终 ckpt 两档（BinFill 补 demo 后；旧 BinFill 因训练/推理不一致作废）

本页是五个模型的**现行权威结果**，同时呈现 50000 步与各自最终 ckpt 两档。2026-09-25 新增 1024（16 帧）与 4096（64 帧）两条感知上下文 run，起跑留档见 [`finalckpt-modul16frame50000-gl/launch.md`](../finalckpt-modul16frame50000-gl/launch.md)。
...
2. **五条 run 在 50k 之后基本收敛**：训到各自最终 ckpt 只带来 −0.7 ~ +2.5 pp，
   排序与量级都没有变化。
3. 三者排序在两档都一致：**512+motion > 2048 > 512**，与补 demo 前 BinFill 上的颠倒排序相反。
4. **感知上下文的收益在 2048 处饱和**：最终 ckpt 档 512 → 1024 → 2048 → 4096 为 32.95% → 41.33% → 45.66% → 45.95%，
   2048 到 4096 只 +0.29 pp；512+motion 的 56.36% 仍比饱和值高 10 pp 以上，说明 motion memory 的收益不是「多看几帧」能替代的。

| 模型 | @50000 | 最终 ckpt | 变化 |
|---|---|---|---|
| 512（8 帧 / budget 512） | 211/692 = **30.49%** | 228/692 = **32.95%**（@59999） | +2.46 pp |
| 1024（16 帧 / budget 1024） | 291/692 = **42.05%** | 286/692 = **41.33%**（@79999） | −0.72 pp |
| 2048（32 帧 / budget 2048） | 312/692 = **45.09%** | 316/692 = **45.66%**（@79999） | +0.58 pp |
| 4096（64 帧 / budget 4096） | 313/692 = **45.23%** | 318/692 = **45.95%**（@79999） | +0.72 pp |
| 512+motion | 378/692 = **54.62%** | 390/692 = **56.36%**（@79999） | +1.73 pp |

macro 分别 512 0.3050→0.3295、1024 0.4202→0.4128、2048 0.4498→0.4566、4096 0.4524→0.4589、512+motion 0.5470→0.5635。error 每档每模型均为 9 条
（五个 run 完全相同的 reset 卡死集，全在 VideoRepick / VideoUnmaskSwap，计入分母按失败处理，故为下界）。
...
| 任务 | 512 | 1024 | 2048 | 4096 | 512+motion |
|---|---|---|---|---|---|
| **BinFill** | 51.4% → 54.9% | 57.0% → 57.0% | 59.2% → 59.9% | 62.0% → 59.2% | 78.2% → 76.8% |
| RouteStick | 35.0% → 39.0% | 62.5% → 61.5% | 75.5% → 80.5% | 78.0% → 82.0% | 83.5% → 83.5% |
| VideoRepick | 26.0% → 24.0% | 31.3% → 32.7% | 30.7% → 28.0% | 25.3% → 26.0% | 42.7% → 48.0% |
| VideoUnmaskSwap | 14.5% → 18.0% | 19.0% → 16.5% | 15.5% → 14.0% | 15.5% → 15.5% | 18.0% → 21.0% |
...
| 难度 | 512 | 1024 | 2048 | 4096 | 512+motion |
|---|---|---|---|---|---|
| easy | 50.3% → 53.8% | 57.8% → 56.3% | 58.3% → 54.3% | 56.8% → 57.8% | 62.3% → 65.8% |
| medium | 36.7% → 39.2% | 53.3% → 52.3% | 56.3% → 53.8% | 52.8% → 52.8% | 63.8% → 64.3% |
| hard | 13.2% → 15.3% | 32.6% → 29.2% | 32.6% → 42.4% | 41.7% → 41.7% | 49.3% → 46.5% |
| xhard | 12.7% → 14.0% | 15.3% → 18.7% | 24.7% → 26.7% | 23.3% → 25.3% | 37.3% → 42.7% |

感知上下文加宽主要抬的是 RouteStick（512 → 4096：39.0% → 82.0%）与 BinFill；VideoRepick 与 VideoUnmaskSwap 四档几乎不动
（VideoRepick 24–33%、VideoUnmaskSwap 14–19%），而这两个任务正是 512+motion 拉开差距的地方（48.0% / 21.0%）。

## 14 组逐格（最终 ckpt 档）

| 组 | 512 @59999 | 1024 @79999 | 2048 @79999 | 4096 @79999 | 512+motion @79999 |
|---|---|---|---|---|---|
| **BinFill/easy** | 41/49 = 83.7% | 39/49 = 79.6% | 39/49 = 79.6% | 42/49 = 85.7% | 45/49 = 91.8% |
| **BinFill/medium** | 28/49 = 57.1% | 31/49 = 63.3% | 30/49 = 61.2% | 29/49 = 59.2% | 45/49 = 91.8% |
| **BinFill/hard** | 9/44 = 20.5% | 11/44 = 25.0% | 16/44 = 36.4% | 13/44 = 29.5% | 19/44 = 43.2% |
| RouteStick/easy | 44/50 = 88.0% | 49/50 = 98.0% | 50/50 = 100.0% | 50/50 = 100.0% | 50/50 = 100.0% |
| RouteStick/medium | 23/50 = 46.0% | 46/50 = 92.0% | 47/50 = 94.0% | 50/50 = 100.0% | 47/50 = 94.0% |
| RouteStick/hard | 10/50 = 20.0% | 26/50 = 52.0% | 42/50 = 84.0% | 44/50 = 88.0% | 39/50 = 78.0% |
| RouteStick/xhard | 1/50 = 2.0% | 2/50 = 4.0% | 22/50 = 44.0% | 20/50 = 40.0% | 31/50 = 62.0% |
| VideoRepick/easy | 9/50 = 18.0%（err 1） | 15/50 = 30.0%（err 1） | 12/50 = 24.0%（err 1） | 12/50 = 24.0%（err 1） | 25/50 = 50.0%（err 1） |
| VideoRepick/medium | 14/50 = 28.0%（err 3） | 16/50 = 32.0%（err 3） | 16/50 = 32.0%（err 3） | 17/50 = 34.0%（err 3） | 19/50 = 38.0%（err 3） |
| VideoRepick/xhard | 13/50 = 26.0%（err 3） | 18/50 = 36.0%（err 3） | 14/50 = 28.0%（err 3） | 10/50 = 20.0%（err 3） | 28/50 = 56.0%（err 3） |
| VideoUnmaskSwap/easy | 13/50 = 26.0% | 9/50 = 18.0% | 7/50 = 14.0% | 11/50 = 22.0% | 11/50 = 22.0% |
| VideoUnmaskSwap/medium | 13/50 = 26.0% | 11/50 = 22.0% | 14/50 = 28.0% | 9/50 = 18.0% | 17/50 = 34.0% |
| VideoUnmaskSwap/hard | 3/50 = 6.0% | 5/50 = 10.0% | 3/50 = 6.0% | 3/50 = 6.0% | 9/50 = 18.0% |
| VideoUnmaskSwap/xhard | 7/50 = 14.0%（err 2） | 8/50 = 16.0%（err 2） | 4/50 = 8.0%（err 2） | 8/50 = 16.0%（err 2） | 5/50 = 10.0%（err 2） |
...
- **1024 / 4096 两条 run 的两档都是 692 条一次跑完**（2026-09-25），无沿用。64 帧 run 的 `MMEVLA_ENC_CHUNK` 与其他 run 一样不设；
  起跑前的显存冒烟只覆盖 SigLIP 编码（1108 帧 6.0 s 不 OOM），4096 token 主干序列由正式 40 片零 OOM 实证。
...
**1024 / 4096 两档**：`finalckpt-modul{16,64}frame{50000,79999}-gl/records/merged/`，manifest、预标清单与 40 片两道闸汇总在
`finalckpt-modul16frame50000-gl/records/`。
