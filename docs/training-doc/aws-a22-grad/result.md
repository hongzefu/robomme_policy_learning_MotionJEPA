...
- **判定行**（`records/aws-a22-grad-compare.log`）：
...
- **records/**：`grad_summary.head.json`、`grad_summary.base.json`（含逐叶 sha256、`stats` 的 max_abs / l2、`batch_keys` 摘要）、`aws-a22-grad-compare.log`、`driver_summary.log`。
