# motion-t3-closed（T3 真实训练端到端，closed 侧 1000 步，保存最终 EMA checkpoint）launch
...
- **run_name**：`motion-t3-closed`；`EXP_NAME=RUN_TAG=motion-t3-closed`；history config **固定** `perceptual-framesamp-context.yaml`（红线 16，不手改开关）。
...
```bash
tmux new-session -d -s motion-t3-closed "set -o pipefail; cd /data/hongzefu/robomme_policy_learning_MotionJEPA; echo HEAD=\$(git rev-parse HEAD); \
  ...
  BENCH_SAVE_FINAL_CKPT=1 BENCH_FINAL_STEP=999 EXP_NAME=motion-t3-closed RUN_TAG=motion-t3-closed \
  ...
  bash scripts/training/g0/run_2gpu_epoch_bench.sh 2>&1 | tee v1-store/logs/motion-t3-closed-driver.log; echo \"EXIT_CODE=\$?\" >> v1-store/logs/motion-t3-closed-driver.log"
```
