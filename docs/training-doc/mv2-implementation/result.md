...
**后续更新**：clean CAND `c0be292c40c4a971161e7233540f4dc5c0c1c7cc` 的 V1/V2/V6/V7 已全部逐位通过，见 [真实 100 步结论](../mv2-v7-guard-cand/result.md)。另外，V5 的合成输入 GPU 单步预检未通过“motion 梯度非零”检查：真实 pi05 结构的随机初始化 AdaRMS 门为零，memory 分支又只进入受该门控制的 FFN；生产训练会先加载 pi05_base。已询问保留 gemma_150m 替身并加载形状兼容预训练参数，或改完整生产模型，尚待用户回复；不把该预检当作 V5 正式通过，也不修改生产模型的门值。
