...
公开 `Yinpei/robomme_data_h5` 全部16个H5；原始实体根为 `/scratch/hongze/robomme_data_h5`。
...
16任务各100集，按已提交全集manifest的规范H5顺序与raw_ep_idx顺序构建；包含demo与execution，执行样本量为 Σ(num_timesteps − exec_start_idx)。
...
source的pkl和逐帧features保留当前生产链；packed为 `framesamp-4x4-v1`，不以训练主机dtype许可改变pack内容。自算 `v1-store/train-assets/mme_vla_suite/16task-pub-1600ep/robomme/norm_stats.json` 只与原版比较，不替换原版训练资产。 所有其他默认值由实际Beta源码锁定，不静默覆盖。
...
完成本库数据验收后，重算剩余磁盘预算再进入 `4task-counting-pub-400ep`；两库全覆盖子集比较和训练前置另行验收。 训练可读性、输入对拍、训练标量/状态及perf仍有自己的闸门，本档案未运行这些项目，不标通过。
...
完成本库数据验收后，重算剩余磁盘预算再进入 `4task-counting-pub-400ep`；两库全覆盖子集比较和训练前置另行验收。 原始H5永久保留；新库和失败现场不自动删除。任何本轮临时大产物清理须先确认归档完整和归属。
