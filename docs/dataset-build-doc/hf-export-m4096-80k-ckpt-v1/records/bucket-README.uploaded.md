# robomme-vla-modul-4096-80k-v1 — v2-1600ep-m64x8x8-modul-b128-80k checkpoint 备份

本 bucket 存放训练 run **`v2-1600ep-m64x8x8-modul-b128-80k`** 的全部 16 个 orbax checkpoint
（316 文件 / 190,052,719,306 B，约 177 GiB），由 `scripts/dataset/hf_export/run_m4096_80k_ckpt_export.sh` 上传，上传前 / 上传后
两遍独立 sha256 逐行对照验收（`SHA256SUMS.pre.txt` 与回读侧重算的 post 清单 diff 为空）。

## 这条 run 是什么

**把 perceptual memory 的 token 预算提到 4096** 的生产训练：`integration_type: modulation`，
perceptual memory 走 **64 帧 × 8×8 frame sampling**（`budget: 4096` = 64 帧 × `token_per_image: 64`），
**`motion` 通道关闭**（`motion_provenance.json` 的 `motion_enabled` 为 `false`、`encoder` 为 `null`）。
80,000 step 跑满，`EXIT_CODE=0` 正常退出，完成器判定
`RUN_COMPLETED=PASS run=v2-1600ep-m64x8x8-modul-b128-80k budget=4096 steps=80000 checkpoints=16 final=79999`。
除 `budget` 外与 2048 那条（`robomme-vla-modul-2048-80k-v1`）配置逐字段一致，是 0922 预算扫描
「4096 → 1024」顺序训练的第一档。

它与同账号其他 checkpoint bucket 的关系如下。**只有与 2048 那条是单变量对照**（只差 `budget`），
与其余各条都不是，不可混作一组读：
...
| **本 bucket** | v2-1600ep-m64x8x8-modul-b128-80k | modulation | **64 / 4096** | off | 80k × 128 |

与 2048 那条的**唯一配置差异是 history YAML 的 `budget: 2048 → 4096`**：两份 YAML 解析后逐键比较
...
（`2048..2067` → `4096..4115`），所以两条的 loss 不要求相等。本库每个执行样本的全域帧号都 ≥ 100，
4096 档每样本恰好选满 64 帧，补零分支在真实数据上碰不到。
...
| history config | `perceptual-framesamp-modul-64frame-8x8.yaml` |
| history config raw sha256 | `e5274831d7d9546c6eab69a047af4aa8ecb7231bb8dba2d173f5ccf84dc544f8` |
...
| streaming_obs_horizon / budget / token_per_image / num_views | 16 / **4096** / 64 / 1 |
...
| 起止（UTC） | `2026-09-23T07:15:33Z` → `2026-09-24T09:51:29Z` |
| wandb run id | `iuw13maz`（project `robomme-framesamp`） |
...
经 `run_modul_budget.sh` 起跑，起跑前通过输入/模型/20 步对拍/100 步保存加载验证、八卡 20 步容量检查与
1000 步测速（报告 `READY`，稳态步时 1.236 s、八卡 GPU 利用率均值 88.40%、0% 采样占比 0.42%）。
...
起跑 `2026-09-23T07:15:33Z`，退出 `2026-09-24T09:51:29Z`，耗时 **26 小时 35 分 56 秒**（95,756 秒，含初始化、checkpoint 保存与退出）。
`EXIT_CODE=0`（正常退出）。
...
`log_interval=100`，除 step 0 外均为日志区间统计。最后一条 loss **0.0011 属于 step 79900**，覆盖训练 step 79801–79900，**不能称作 step 79999 的单步 loss**；step 79901–79999 共 99 步没有独立标量日志。**训练 loss 不代表策略评估成功率。**

## 完整性锚点

- 16 份 `assets/robomme/norm_stats.json` 的 sha256 **全部相同**，为
  `856c75ea504bd104c552027987b98a512d2d0b406738a7a8a500ada96d8ed173`
  ——与 `robomme-vla-modul-2048-80k-v1`、`robomme-vla-modul-60k-v1`、`robomme-vla-modul-motion-80k-v1`
...
_run-meta/
  ...
  wandb/run-<ts>-iuw13maz/                                # wandb 本地 run 目录（含 .wandb 事件流）
...
本 run 不写 tensorboard，训练曲线在 `_run-meta/wandb/` 里（也可在 wandb 上按 run id `iuw13maz` 查）。

## 恢复与自验

```bash
# 下载整个 bucket（huggingface_hub >= 1.x）
hf sync hf://buckets/HongzeFu/robomme-vla-modul-4096-80k-v1 ./robomme-vla-modul-4096-80k-v1
cd robomme-vla-modul-4096-80k-v1

# 校验下载完整性（覆盖除清单自身外的全部字节）
sha256sum -c --strict SHA256SUMS.pre.txt

# 只取最后一个 checkpoint（约 11 GiB）
hf sync hf://buckets/HongzeFu/robomme-vla-modul-4096-80k-v1 ./ckpt-79999 --include '79999/*'
```
...
params = ocp.PyTreeCheckpointer().restore("robomme-vla-modul-4096-80k-v1/79999/params")
```
