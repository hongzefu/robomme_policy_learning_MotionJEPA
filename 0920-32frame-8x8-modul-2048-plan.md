...
> 创建日期：2026-09-20（America/New_York）。状态：待实施。本轮授权仅为在根目录新增本文，不修改代码、配置、数据或训练超参，不启动训练。
>
...
| 新增 `scripts/training/tests/check_32frame_modul.py` | 专门验证 32×64 无 motion 的独立选帧、输入装配、在线预缓存特征装配和恢复形制；不用 motion 库作测试前提 |
| 新增 `scripts/training/tests/check_modul_train_records.py` | 严格核对两侧预期更新次数、输入索引、初态、更新后状态、叶集合与有限性，再做逐位比较；不改历史验收脚本的既有门槛 |

新检查器的拟议接口为 `--store`、`--source`、`--manifest`、`--norm-stats`、`--history-config`、`--out`；若承载 checkpoint 恢复检查，再增加明确的 `--checkpoint` 模式。**这些是待实现接口，不是当前可执行命令。** 源 NPY 参考读取不调用候选 Dataset 的 `_pad` 或读取器，避免两侧复制同一个错误。
...
| 新 2048 训练与恢复 | 真实 packed 数据，生产训练入口，独立新 run | 100 | loss/梯度/更新后参数有限，观察窗口内记忆及 modulation 分支存在非零梯度和实际更新；保存 checkpoint 后按快照恢复成功，动作 shape 正确且有限 |
...
`train.main` 会使用 `MMEVLA_JAX_CACHE_DIR` 重设编译缓存；仅设置 `JAX_COMPILATION_CACHE_DIR` 不足以防止回退到 `$HOME/.cache`。真实保存/恢复短测也必须显式设置该项目内缓存路径。
...
取证工具会把 checkpoint 保存替换为状态摘要，因此它不能证明 checkpoint 可恢复。表中最后一项单独走 `scripts/training/train.py` 真实保存，再通过 `create_trained_policy` 加载保存的 checkpoint 和快照，验证完整训练/恢复路径。
...
同时核对 norm_stats 的文件 SHA 与解析后数组摘要，保存 `history_config.resolved.yaml`、其 SHA256 和 `motion_provenance.json`。新 run 的 checkpoint 恢复必须实际得到 2048 长度、motion 关闭；不覆盖旧 run，不修改旧快照。

## 8. 交付与完成判据

本轮交付仅为根目录的 `0920-32frame-8x8-modul-2048-plan.md`。文档完成后检查 Markdown 代码围栏、相对链接、`git diff --check` 与文件范围，仅逐文件暂存本文，以中文 `docs:` commit 提交，再按当前分支既有 upstream 执行 `git push`。不提交其他任务的在途文件。

后续实施按“验证工具准备并固定基线 → 新配置与守卫 → 非训练输入验证 → 真实训练一致性 → 2048 保存/恢复”顺序推进。代码交付应包含新 YAML、明确限制在本目标内的 Dataset 支持、必要测试及结果档案；每个阶段按实际代码与验证结果提交，不把本方案中的预期值写成已完成结果。

只有新配置通过守卫、真实 2048 输入对拍通过、旧 512 档回归通过、真实训练更新与 checkpoint 恢复通过后，才可把结论更新为“支持 32 帧 × 8×8、2048 token 的无 motion modulation 训练”。正式大 batch 性能、长训练收敛和策略成功率由各自后续运行提供证据。
