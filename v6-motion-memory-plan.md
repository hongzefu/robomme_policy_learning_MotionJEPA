...
> **本文件是 v6 工作的权威计划**（2026-09-01 **第二版**，经用户五轮澄清逐条定稿，待批准实施）。
> 第一版（commit `132ca7f`）的运动路口径已被用户澄清整体推翻——原稿把 motion token 与
> framesample 的 32 个采样帧一一对齐、并推荐「后视窗口」，本版改为**独立的段内绝对网格采样 +
> 前视窗口 + 尾端不越当前帧**；`missing_motion_emb` 设计随之作废，改用与 `static_mask` 同款的
> mask。第一版中经核对无误的部分（两库同源性、格式契约体例、红线、闸门框架）保留。
> **锚点**：分支 `v1-dataloader-Restructure`，HEAD = `132ca7f`（工作区 clean）。
...
## 二、用户拍板汇总（2026-09-01 会话，五轮澄清）

| 议题 | 拍板 |
|---|---|
| 特征来源 | MotionJEPA 两级链路：Wan VAE（离线冻结）→ `WanLatentMotionEncoder` |
| 接入形态 | 「**作为 memory 的一部分**」——记忆序列的第二路，不是插单个 token 进 prefix |
| framesample | 「**逻辑不变，你不用管**」——帧路 `even_sampling_indices` 一字不动 |
| 起点定义 | 「以 VLA 训练时每个 action chunk 的开始作为 f」「间隔一个 action chunk 抽取一次」 |
| 窗口约束 | 「训练时候 f+32 窗口必须小于等于当前帧」——**前视窗口，尾端不越当前帧** |
| 起点网格 | **段内绝对网格（B）**——起点钉死在 `段起点 + 20m`，不随当前帧平移（见 5.1 数轴） |
| 8 帧空白 | 「**采样不到 33 窗口都不补**」——不加网格外起点，不做钳位回退 |
| demo/exec | 「**独立的，也是一样采样不到 33 窗口都不补**」——各自成网格，窗口不跨界 |
| 间隔 | **独立配置键 `motion.stride`，默认值写 20（= 当前 `action_horizon`），但不自动跟随** |
| 预算 | 「**尽可能不截断任何样本**」「**尽可能和 framesample 现在对齐**」「**使用 32**」→ **N=32** |
| 填充率代价 | 用户明确要求「**写入 plan 让用户清楚认知**」→ 见 5.4 |
| 三个「无偏好」项 | 窗口方向 / 编码器承载 / 注入点三问用户答「无偏好」，授权按推荐定稿 |

## 三、方案总览

**一句话**：memory 从「512 个外观 token」变成「512 个外观 token **并列** 32 个运动 token」，
prefix 记忆区 512 → **544**。运动路的采样与帧路**完全独立**：帧路照旧变长间隔铺满全历史，
运动路按段内绝对网格每 20 帧取一个起点、每个起点往后 33 帧编一个运动向量、窗口尾端不得越过
当前帧。训练读离线表，在线评估每 20 帧增量现编一次。

五个已定死的口径，依据分别在 5.1–5.5：
...
### 4.2 起点集合与索引映射（段内绝对网格）

设当前样本的全 timestep 域帧号为 `t`，该 episode 的 `exec_start_idx = es`：
...
⚠ **两段各自成网格、互不延续，窗口一律不跨 demo/exec 边界**（latent 分段抽，跨界窗口不存在）。
⚠ 起点集合与 `even_sampling_indices` 选出的 32 个帧号**没有任何对齐关系**——两路各采各的。

## 五、五个口径的原理与实测依据

### 5.1 为什么起点用段内绝对网格（B），不用相对当前帧的滑动网格（A）

训练样本的当前帧 `t` 是**逐帧 dense** 的，多数不落在 20 的倍数上，于是「间隔一个 action chunk」
有两种读法。下面用段起点 = 0、当前帧从 **205 走到 206** 的实际数轴对照：
...
**差别的本质**：A 的起点是相对 `t` 定义的，`t` 一动整排跟着动，历史算过的全部作废；
B 的起点是绝对位置，`t` 只是把可见边界往右推，已经算过的永远有效。
...
窗口是**前视**的——起点往后 33 帧，约束「尾端 ≤ 当前帧」。窗口 33 帧比 chunk 步长 20 帧长，
所以当前帧附近必然留一段空白：网格上最靠前的合法起点，其窗口尾端离当前帧最近 8 帧、最远 27 帧
（取决于 `t` 落在网格的哪个相位）。

**用户已拍板：不补。** 不加网格外的起点（例如紧贴当前帧的 `t−32`），不做钳位回退。凑不齐完整
33 帧窗口的位置就是缺失，走 padding + mask。

### 5.3 预算 N=32：零截断

段内绝对网格下，合法起点个数实测（395,289 个样本全量）：
...
两路同预算、形制对齐（用户要求的「尽可能不截断」+「尽可能和 framesample 对齐」同时满足）。

### 5.4 ⚠ 零截断的代价：平均只有 8 个位置是真数据（用户要求显式认知）

**这是本方案最需要清醒认识的一点，不藏在技术细节里：**
...
### 5.5 为什么并列拼接 + mask，而不是交错 / 特征维融合 / missing embedding

**并列 vs 交错 vs 特征维融合**：

| 方式 | 序列形态 | 评价 |
|---|---|---|
| **A. 并列拼接（选定）** | `[512 帧路] + [32 运动路]` = 544 | 运动路完全独立：独立采样、独立投影、独立 mask，`motion.enabled=false` 一键退回**逐位等价**的旧链路 |
| B. 交错 | 每帧 `16 + 1` | 两路采样集合根本不同（帧路变长间隔、运动路绝对网格），**没有可交错的对应关系**——本轮口径下直接不成立 |
| C. 特征维融合 | 每个 image token concat motion | 同上，无对应关系；且要把 `encoder_static` 从 `Linear(2816→2048)` 拓宽 |

第一版曾把 B/C 作为备选，那是建立在「两路一一对应」的旧口径上；用户澄清两路独立采样后，
**B 和 C 已不成立**，只剩 A。

**mask vs missing embedding（本版反转，第一版设计作废）**：

第一版定「运动段恒占位、`input_mask` 恒 True、缺失位喂可学习的 `missing_motion_emb`」，理由是
避免 `positions = jnp.cumsum(input_mask, axis=1) - 1` 造成的位置抖动。**在 25.3% 填充率下这个
设计站不住**：平均 24 个位置会是同一个可学习向量重复几十遍，6.48% 的样本更是 32 个位置全是它，
attention 会被严重稀释。

改为与 framesample 的 `static_mask` **完全同款**——padding 位 `motion_mask=False`。位置抖动确实
存在，但那是 framesample 早已有的既有语义（它在 `step<32` 时同样 padding + mask），
用户要求的「尽可能和 framesample 对齐」也指向这一边。`missing_motion_emb` 随之删除。
...
**运动段为什么放在 img 之前**：`make_attn_mask` 的
...
`motion_pos_proj` 与 `motion_encoder_static` 两个新投影。

## 七、在线同步推理的延迟账（风险已因 B 网格大幅降级）
...
| **S2 model 接线** | 双路 memory + `motion.enabled` 开关 + `motion_mask`（本计划主体） | 关闭态与 HEAD **逐位等价**；开启态 smoke 跑通；`‖motion_tok‖/‖mem_tok‖` 同量级；有效数分布与 5.3 一致 |
| **S3 在线接线** | `FrameSampMemory` 绝对网格增量编码 + Wan VAE 常驻 + 尖峰处理 | 在线/离线同一起点特征一致；端到端 ms/step 实测 |
| **S4 消融** | ① 预算 N（32 / 24 / 16） ② 叠加 adaRMS 调制 ③ 运动段放 img 之后 ④ `motion.stride` ⑤ 冻结 vs JAX 移植微调 ⑥ **按有效数分桶的分层评估**（5.4 后果 3） | 训练曲线 + 在线成功率 |
...
7. **第一版已作废的两处设计，禁止从 git 历史翻回**：① motion 与 framesample 采样帧一一对应；
   ② `missing_motion_emb` + 恒 True 的 `input_mask`。理由分别见 5.1 与 5.5。
...
（第一版的 `missing_motion_emb` **不建**，见 5.5。）
...
- `src/mme_vla_suite/training/framesamp_dataset.py`：`FrameSampDataset.__getitem__` 已有
  `g` 与 `step`，运动路查表**不复用** `frames`（两路独立采样，见 4.2）；起点集合按一节的公式
...
- **尖峰处理**（七节风险 1）：第 20 步的 1.57 s 尖峰若不可接受，可提前一步预编——起点的可见
...
| **M7** 有效数分布 | S2 | 逐 batch 统计 `motion_mask.sum(axis=1)`，分布须与 5.3 实测一致（中位 7、均值 8.09、6.48% 全零） | 不一致即起点集合算错 |
...
| R2 | **填充率仅 25.3%，运动路信号被 padding 稀释** | **高** | **中** | 已在 5.4 显式记账；M7 盯有效数分布；S4 增「按有效数分桶的分层评估」与预算 N 消融 |
