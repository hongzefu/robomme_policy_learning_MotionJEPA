...
实施期最终决定（2026-09-23）：用户原话「可以优化取证方式 但要保证改前后都是用的一种取证方式 这个是用户最终决策」。允许优化取证器，但同一对照组的改前、改后、最终记录开关两侧及各自1步补跑，必须使用同一套取证源码、线程数和记录设置。`check_modul_train_records.py::recorder_identity` 从记录中的完整提交还原取证器源码摘要，`same_recorder` 在拼接补跑和比较两侧时强制相同；不同即拒绝，统一方法后重新取证。保持原有20+1步、201叶、逐位一致判据。a批次串行前缀只用于取证器协议检查，不与b批次八线程记录拼成训练代码等价结论。只修验收工具时，可按 `Workflow.reuse_baseline` 的代码白名单、环境及数据指纹检查，复用完整三组并用新验收器重读全部原记录；取证源码或设置改变则该复用入口拒绝。
