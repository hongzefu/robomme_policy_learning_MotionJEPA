...
### 决策门结论（2026-08-28 回填，v1-95util L0 收官）

**走第一条分支：util 已达 95%+，三个加速项全部不立项。**

GL 生产口径实测（数字权威源见 v2 计划 T8 的 S0 与 S0 复验两行）：双 seed/双节点
`E2E95_ACCEPT` 双 PASS，util 均值 **99.7% / 99.9%**，0% 采样 ≤0.125%，步时 4.756 / 4.647 s。
距 100% 的残差不足 0.3pp，项 2、3 主张的「省 IPC/传输 40%」在生产口径下无可压缩空间。

三条具体裁定：

- **项 1（worker JAX 导入链）不立项，且其现象前提已被证伪**：v1-95util 诊断实测 GL 上
  全程只有主进程一个 pid，「每 worker 在 GPU0 建 442 MiB CUDA context」不成立
  （原始存证见 `docs/training-doc/v1-framesamp-e2e/records/` 的 `compute_apps.csv`）。
  该项从补救清单撤下。
- **项 2（pos_emb 移出 batch）、项 3（`static_state_emb` 清除）不立项**：性能动机消失。
  若将来因**可读性/正确性**理由重提，须按各自节内的迁移矩阵与前置条件独立立项，
  不得援引本轮性能数据作依据。
- **关键归因（决定上述裁定的事实）**：此前 80–90% 一族数字系**量具每步强制
  `jax.device_get` 打断流水线**所致，非数据供应瓶颈；同配置同节点仅改日志粒度，
  util 88.1% → 99.7%。原决策门「util 卡在 80–90%」这一分支的触发前提从未真实成立。
