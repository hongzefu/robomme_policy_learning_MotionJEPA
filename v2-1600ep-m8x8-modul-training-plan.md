...
**锚点为什么定在 `07702f0`（2026-08-29）**：本来只打算比 motion 接入前后那两个版本。核实之后发现可以推得更早 —— 而且**再往前推就没意义了**：模型代码从 2026-02-20 到 2026-08-29 整整**半年没动过**（那半年的差异只有三个文件各一行，都跟这件事无关，而且后来都被删了）。所以一次对拍就覆盖了整条演进链，motion 接入只是其中一个 commit。往更早推只会增加适配成本，换不到任何新信息。
