...
**G3 之后另有追加的统一入口工作**（2026-08-29 用户显式批准；原编号 commitV3.15，2026-08-30 改定 commitV5.0）：`scripts/training/main.py` 统一入口与入口等效性验证。它不在上表七刀序列内、不改训练链任何文件、不影响 G3 判据，权威文档为仓库根目录 `0830-train-entry-restructure-plan.md`（文末第三部分已缩为指针；2026-08-30 二次改向：不再新增 main.py，改为 train.py 本体单跑收敛 + bench 拆分 g0/util + 双重对拍，详见该文件）。
...
# 第三部分（已拆出）：训练入口工作 —— 见 `0830-train-entry-restructure-plan.md`
