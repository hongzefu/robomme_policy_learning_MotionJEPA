...
### 第二块：本机启动训练，前 300 步梯度一致（最后检验）——G1 vs G0

本块即基线链的 **G1 vs G0** 对拍（符号以基线计划「符号总表」为权威）：**G0** = `v1-grad-baseline-g0` 黄金基线固化产物（三计划均未实施的原始训练语义，兼任本计划修复前一侧，不重跑）；**G1** = `v1-dtype-ab-post`（修复后 B 侧，本计划现跑）。本机 2×RTX 6000 Ada、b8、300 步（用户拍板；b8 下含短样本 batch 占 40.4%，300 步期望命中约 121 次差异场景）：
...
3. 300 步 G1 vs G0：G1 起跑前必过 `BASELINE_ENV` preflight，并应尽快接续 G0（理想同场次——跨期复用能力是给无法重跑 A 侧的后续链节用的，不是推迟 G1 的理由）。逐步比 loss/grad_norm/llm_grad_norm/mem_enc_norm/param_norm 五个标量 hex + 每 100 步（步 0 与末步必记）完整 TrainState 摘要。主判据 bitwise；因 G0/G1 输入 dtype 不同、XLA 编译产物不同，bitwise 存在虚假失败的可能——预设量化判据兜底（等价性检验形态，权威版本见基线计划「量化判据」节），启用前须先证 G1 自身重跑稳定。
...
- 两块正确性全部通过后，新增 **P7 跑 `v1-g1-speed`**（G1-speed；生产 XLA 档、`SAVE_INTERVAL=0`、`BATCH_DIGESTS=0`、300 步 b8 seed 42 num_workers=4——口径以基线计划「符号总表」的 speed run 统一口径为权威），vs 速度基线 `v1-g0-speed`（speed 链锚点 1.117 s/step，已随基线计划 PG0-speed 步骤在修复前预跑）产出 dtype 修复的单独性能对比。
...
| P6 | 300 步 G1（B 侧）+ 与 G0 固化产物对拍（纯正确性，util 数据仅留档参考） | 基线计划 PG0 固化产物 + P5，起跑前 `BASELINE_ENV=PASS` | bitwise（或量化兜底）PASS | ~1 h |
| P7 | G1-speed 性能 run `v1-g1-speed`（生产 XLA 档，符号总表口径；2026-08-26 反转裁定新增，见第四节） | P6 PASS（两块正确性全过） | run 完成 + AGENTS 16 稳态统计 + vs `v1-g0-speed` 对比报告 | ~40 min |
...
- **run_name 与留档（2026-08-26 审计修正：P3–P5 不再按短程豁免）**：P3/P4/P5 预计 15–30 min，均超 5 min，按 AGENTS 17 视作完整运行——各自从 clean HEAD 起跑（P3 于 V2.4a、P4/P5 于 V2.4b）、唯一 run 名（`v1-dtype-p3-dump-pre` / `v1-dtype-p4-cmp` / `v1-dtype-p5-grad`，起跑前确认）、留档 `docs/training-doc/<run 名>/`（launch.md、result.md、records/）。300 步 B 侧 `v1-dtype-ab-post` 照旧（`v1-dtype-ab-pre` 不单独跑，A 侧由 G0 兼任正确性一侧）；P7 的 `v1-g1-speed` 按 speed run 规约留档 `docs/training-doc/v1-g1-speed/`、以 `docs:` commit 提交不占 V2.x（基线计划符号总表），起跑前仍按 AGENTS 6 确认 run_name；前置的 `v1-det-*`、`v1-grad-baseline-g0`、`v1-g0-speed` 由基线计划确认与留档，本计划不重复。
...
| D4 | **性能数字禁止取自带 TrainState 摘要 / batch_digests / 确定性 XLA 档的 run**（正确性 run 的 util/步时仅留档参考）；本计划的性能结论**仅**出自 P7 `v1-g1-speed` vs `v1-g0-speed`（speed 链，基线计划符号总表口径），本机数字标注与 util 判读按 AGENTS 13/16 |
