...
#     "bash scripts/data-pack-framesamp/run_pack.sh"
# 可调环境变量：
...

{
  ...
  uv run --project "${REPO_ROOT}" python \
    scripts/data-pack-framesamp/pack_framesamp_store.py pack \
  ...
  uv run --project "${REPO_ROOT}" python \
    scripts/data-pack-framesamp/pack_framesamp_store.py verify \
