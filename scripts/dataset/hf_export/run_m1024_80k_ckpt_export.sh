#!/usr/bin/env bash
# driver：把训练 run v2-1600ep-m16x8x8-modul-b128-80k（modulation + **motion 关闭**、
# 16 帧 × 8×8 = budget 1024）的全部 16 个 orbax checkpoint
# （5000…75000 共 15 个 + 收尾 79999，327 文件 / 190,050,687,155 B ≈ 177 GiB）
# 上传到 HF **private** bucket HongzeFu/robomme-vla-modul-1024-80k-v1，
# 并做「上传前 / 上传后」两遍独立 sha256 逐行对照验收。
#
# 蓝本是同目录 run_m4096_80k_ckpt_export.sh（0922 预算扫描「4096 → 1024」的前一档，
# 同一 config 条目、同样 motion 关闭、同样 16 个 checkpoint 与完成器产物），逻辑逐行相同，
# **只换 run / budget / bucket 名、train-cmd 的 history YAML（16frame）与 README 模板**。
# 蓝本相对 2048 那条的四处改动（只走趟 B、按本 run 的 EXIT_CODE=0 与完成 JSON 断言训练已完成、
# EXPORT_THROTTLE 控制限流、_run-meta 附完成证据）与允许从 -temp 开发副本执行的 STORE 解析全部沿用。
# 1024 训练结束后同机无训练在跑，起跑时用 EXPORT_THROTTLE=0（不限流、NPROC=8）。
#
# 沿用的四条做法（都是本环境实测踩出来的）：
...
# 每 step 文件数不等（ocdbt 的 d/ 分块数随内容变），故不硬编码总文件数。
set -euo pipefail
...
RUN_NAME="v2-1600ep-m16x8x8-modul-b128-80k"
BUDGET=1024
BUCKET_ID="HongzeFu/robomme-vla-modul-1024-80k-v1"
...
} > "$STAGE/_run-meta/git-commit.txt"
{
  ...
  echo "  --model.history-config perceptual-framesamp-modul-16frame-8x8.yaml"
...
RM_SRC="$REPO/scripts/dataset/hf_export/m1024_80k_bucket_README.md"
