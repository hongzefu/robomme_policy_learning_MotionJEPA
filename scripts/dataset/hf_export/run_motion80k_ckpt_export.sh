#!/usr/bin/env bash
# driver：把训练 run v2-1600ep-m8x8-modul-motion-b128-80k（modulation + **motion 开启**）
# 的全部 16 个 orbax checkpoint（5000…75000 共 15 个 + 收尾 79999，约 190 GB）
# 上传到 HF **private** bucket HongzeFu/robomme-vla-modul-motion-80k-v1，
# 并做「上传前 / 上传后」两遍独立 sha256 逐行对照验收。
#
# 蓝本是同目录 run_modul60k_ckpt_export.sh（无 motion 的 60k 对照组，132.76 GiB / 274 文件），
# 六阶段骨架照抄。本脚本与它的**根本差异**：那条 run 已经跑完才开始导出，本条
# **训练还在跑**（num_train_steps=80_000，见 src/mme_vla_suite/training/config.py:576）。
# 因此本脚本设计成**分两趟、可任意重入**：
#
...
#                     pre 清单，然后走完蓝本的阶段 3–6（计数核对 / 全量回读 /
...
#   - **stage 走硬链接**：stage 与源同在 /dev/md0，ln -f 额外占盘为 0，不是再拷 190 GB。
...
#
# 与蓝本的另一处差异：蓝本 rm -rf 重建 stage、硬编码 LOCAL_FILES=274。本脚本要多趟跑，
# stage 改为幂等增量（只 mkdir -p + ln -f，不删），且每个 step 目录落一份**独立的**
# pre 清单分片 tmp/pre-parts/<step>.txt（step 目录写完即不可变，算一次就够），
# 全局 SHA256SUMS.pre.txt 在趟 B 由分片合成——否则中途生成的全局清单会覆盖掉还没传的文件。
...
BUCKET_ID="HongzeFu/robomme-vla-modul-motion-80k-v1"
BUCKET="hf://buckets/$BUCKET_ID"
# 注意中间层是 mme_vla_suite_b128_80k，**不是** _b128_60k（那是无 motion 对照组）
...
TRAIN_LOG="$REPO/v1-store/logs/$RUN_NAME.log"
WANDB_ID_FILE="$SRC/wandb_id.txt"
EXPECT_STEPS=16
EXPECT_LAST="79999"
# 根下 sidecar：训练启动时（05:47）一次性写完的静态文件，趟 A 就可以传
...
# ---------------------------------------------------------------- 训练在跑？
# 这条判定决定了本次是趟 A 还是趟 B，以及要不要限流。匹配 uv run 的 wrapper 与真身两个进程。
...
if [ "$TRAINING_LIVE" = 1 ]; then
  ...
  # 再要求目录内最近一次写入已过 QUIET_S 秒（训练每 ~80 分钟才存一次，静默 10 分钟足够宽）。
  # 两条都满足才收，否则跳过等下一趟——宁可少传一个，不可传半个。
  SAFE=( "${ALL[@]:0:$(( ${#ALL[@]} - 1 ))}" )
  NEWEST="${ALL[-1]}"
  QUIET_S=600
...
# 趟 A 若误 stage，pre/post 清单必然 diff 不为 0。
...
{
  echo "# 训练命令行（趟 B 生成时从训练日志 / 本文件同目录 provenance 抄录）"
  ...
  echo "  --model.history-config perceptual-framesamp-modul-8frame-8x8-motion.yaml"
...
# TODO 哨兵。数字不经人手，README 与日志不可能对不上；生成脚本自己会断言哨兵已被消掉。
# _motion-encoder/：这条 run 实际用的 MotionJEPA encoder 本体。
# 之前只有 motion_provenance.json 里的 sha256 指纹，HF 上没有任何副本
# （模型库 HongzeFu/MotionJEPA 里那份是**上一代** wan-v8-filter10-72ep-a），
# 导致光拿这个 bucket 没法对新数据重算 motion 特征。放进来让 bucket 自包含。
# 上传前先按 motion_provenance.json 里的 encoder.checkpoint_sha256 验一遍，对不上就停。
ENC_SRC="$REPO/v1-store/external/motionjepa/wan-full1600-filter2-b176x4-72ep-a"
ENC_EXPECT="$(python3 -c "
import json
print(json.load(open('$SRC/motion_provenance.json'))['encoder']['checkpoint_sha256'])
")"
[ -f "$ENC_SRC/checkpoint_epoch_72.pt" ] || { echo "错误: 缺少 encoder $ENC_SRC/checkpoint_epoch_72.pt"; exit 1; }
ENC_ACTUAL="$("${THROTTLE[@]}" sha256sum "$ENC_SRC/checkpoint_epoch_72.pt" | cut -d' ' -f1)"
[ "$ENC_ACTUAL" = "$ENC_EXPECT" ] || {
  echo "错误: encoder sha256 与 motion_provenance.json 不符"
  echo "  期望 $ENC_EXPECT"; echo "  实际 $ENC_ACTUAL"; exit 1
}
echo "  encoder sha256 与 provenance 一致: $ENC_ACTUAL"
mkdir -p "$STAGE/_motion-encoder"
ln -f "$ENC_SRC/checkpoint_epoch_72.pt" "$STAGE/_motion-encoder/checkpoint_epoch_72.pt"
ln -f "$ENC_SRC/config.yaml" "$STAGE/_motion-encoder/config.yaml"
hash_subdir "_motion-encoder" "$PARTS/_motion-encoder.txt"

RM_SRC="$REPO/scripts/dataset/hf_export/motion80k_bucket_README.md"
...
# ---------------------------------------------------------------- 阶段 4
echo "阶段4开始：全量回读到 $VERIFY（约 190 GB）"
...
# pre 清单比源多 README.md 与 _run-meta/*（stage 专有）；比对前剔掉并断言剔除条数对得上。
grep -vE '  (README\.md|_run-meta/|_motion-encoder/)' "$STAGE/SHA256SUMS.pre.txt" > "$TMPD/pre.src-only.txt"
...
ENC_LINES=$(wc -l < "$PARTS/_motion-encoder.txt")
echo "  pre(剔 README/_run-meta/_motion-encoder)=$PRE_ONLY_LINES 行, src-after=$SRC_LINES 行, _run-meta=$RUNMETA_LINES 行, _motion-encoder=$ENC_LINES 行"
[ "$((PRE_LINES - PRE_ONLY_LINES))" = "$((RUNMETA_LINES + ENC_LINES + 1))" ] \
  || { echo "错误: 剔除行数应为 $((RUNMETA_LINES + ENC_LINES + 1))，实为 $((PRE_LINES - PRE_ONLY_LINES))"; exit 1; }
