...
```bash
# 布局探针（slice 档前提复核）
UV_LINK_MODE=copy uv run python scripts/dataset/pack/probe_layout.py \
  --source v1-store/datasets/4task-gl

# 切分预览（不写盘）
UV_LINK_MODE=copy uv run python scripts/dataset/pack/pack_framesamp_store.py plan \
  --manifest v1-store/episode_manifest.json

# S4 全量打包 + 全量校验（detached tmux，AGENTS 7）
tmux new-session -d -s pack-framesamp "bash scripts/dataset/pack/run_pack.sh"

# Store 组守卫
UV_LINK_MODE=copy uv run pytest scripts/dataset/pack/test_pack_guards.py -x -q
```
