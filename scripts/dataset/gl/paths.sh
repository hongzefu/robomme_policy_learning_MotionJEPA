#!/usr/bin/env bash
# v1 集群数据处理链路（scripts/dataset/gl/）的**唯一**路径与环境源。建库域脚本
# 一律 source 本文件，不再各自硬编码路径；训练域（scripts/training/）自带同构的
# paths.sh，两域互不引用（commitV4.6 目录统一后口径）。
#
# 设计要点：
...
# ⚠ 集群侧看不到本机 /data：从本机副本提交 Slurm 作业不可用（gl_submit.py 的 REPO 仍指 turbo）
...
# 根自证断言（fail-loud，计划二节第 11 条）：搬深一层后层数若数错，turbo 前缀
# 校验挡不住（<repo>/scripts 同样在前缀下），必须以 pyproject.toml 存在性判根，
# 否则 v1_prepare_dirs 会在错误位置静默造出整棵假 v1-store 目录树
