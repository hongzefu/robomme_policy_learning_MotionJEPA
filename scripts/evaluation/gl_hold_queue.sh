...
#   CHUNK_EPISODES SEED ALLOW_RESUME MMEVLA_MOTION_PROV_RELAX；STEP_TIME（每个 step 的 --time，默认 12:00:00）
...
for V in POLICY POLICY_CONFIG EXPECT_CKPT_ID EPISODE_WALL_S EVAL_TIMEOUT CHUNK_EPISODES SEED ALLOW_RESUME MMEVLA_MOTION_PROV_RELAX; do
    [[ -z "${!V:-}" ]] || PASS+=("$V=${!V}")
done
FAILED=0
...
for K in "$@"; do
    ...
    srun --jobid="$JOB" --account=chaijy2 --partition=spgpu --gpu_cmode=shared --overlap --exact \
        ...
        /usr/bin/env -u ROBOMME_GPU_RASTER -u SAPIEN_DISABLE_RAY_TRACING PYTHONUNBUFFERED=1 "${PASS[@]}" SHARD_INDEX="$K" \
