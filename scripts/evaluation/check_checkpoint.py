...
按 run 根目录名（checkpoint 的父目录名）查 RUNS 表，决定 step、motion 开关、config 名与两个 sha256 的期望值；
...
RUNS = {
    ...
    'robomme-vla-modul-60k-v1': dict(
        step='59999', motion=False, config='mme_vla_suite',
    ...
    'robomme-vla-modul-motion-80k-v1': dict(
        step='50000', motion=True, config='mme_vla_suite_b128_80k',
...
assert checkpoint.name == run['step'], (checkpoint, run['step'])
...
print(f"HISTORY_CONFIG=PASS NORM_STATS=PASS {'MOTION_ENABLED=PASS' if run['motion'] else 'MOTION_DISABLED=PASS'} config={run['config']} step={run['step']}", flush=True)
