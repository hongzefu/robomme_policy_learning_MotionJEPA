...
[[ "$QUEUE_DIR" == /* && -f "$QUEUE_DIR/items.txt" ]] || { echo "队列不存在：$QUEUE_DIR/items.txt"; exit 2; }
DRY="${DRY_RUN:-0}"
...
export STEP_TIME=12:00:00 EXPECT_CKPT_ID=50000 EPISODE_WALL_S=2400 EVAL_TIMEOUT=14400 SEED=7 \
...
echo "WORKER_START job=$JOB node=$NODE head=$HEAD queue=$QUEUE_DIR suffix=${SUF:-none} chunk=$CHUNK_EPISODES $(date -u +%FT%TZ)"
RAN=0; FAILED=0
while true; do
    ITEM=""; MODEL=""; K=""
    ...
    T0="$(date -u +%FT%TZ)"
    if [[ "$DRY" == 1 ]]; then
        echo "DRY gl_hold_queue.sh $JOB $RUN $CKPT $REPO/v1-store/evaluation/$RUN/plans $HEAD $K POLICY=$POLICY POLICY_CONFIG=$POLICY_CONFIG"
        sleep 1; RC=0
    else
