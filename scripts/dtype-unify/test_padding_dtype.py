...
2. **哈希口径守卫**——本目录 `_common` 的 raw / canonical 摘要必须与
   `scripts/smoke-local/bench_train_steps.py` 的同名实现逐字节同结果。两处口径一旦
   漂移，G1 vs G0b 的输入侧对拍就会拿两把不同的尺子量同一件事。
3. **位型容器 round-trip**——覆盖 bf16 / f32 / f64 / bool 全部出现的类型。

跑法：`UV_LINK_MODE=copy uv run pytest scripts/dtype-unify/ -q`
"""
...
def _load_bench_module():
    p = C.REPO_ROOT / "scripts" / "smoke-local" / "bench_train_steps.py"
