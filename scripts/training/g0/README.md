...
| `bench_train_steps.py` | 训练入口：只调一次 `train.main(config)`，训练循环一行不改；靠 monkeypatch 把逐步标量、完整 TrainState 摘要（params/opt_state/EMA/step，步 0 必记）、输入 batch 摘要（raw + canonical 双口径，P1b）与样本 index 序列写成 jsonl/json，另留档编译缓存事件计数与真实 argv。速度口径开关：`SAVE_INTERVAL=0` 禁 TrainState 摘要并默认联动禁输入摘要（P1b；`BATCH_DIGESTS` 显式设置可覆盖，speed 链 run 专用，见 `0827-framesamp-restructure-plan.md` 符号总表）。`EXTRA_DIGEST_STEPS` 加记附加摘要步、`STATE_DUMP_STEPS` 在指定摘要步把 TrainState 数组按位落盘（逐叶数值裁决参照，P1b）。`BENCH_DUMP_IDX=1`（S0'，v2 计划 C.1）batch_sampler 层逐 batch 记 index 到 `idx_seq.jsonl`（端到端旁证，含 prefetch 超前；默认关） |
...
官方口径 = `scripts/finetune_mme_vla_suite.sh` → `scripts/training/train.py` 的 `mme_vla_suite`
...
- **2026-08-26 四档确定性实验定档（D0/D1/D2/D2-cold 各两轮 100 步，commit V2.1，
  留档 `docs/archive/training-doc/v1-det-*/`，权威结论见 `docs/archive/v1-phase1-gradient-baseline-report.md` 九节）**：
