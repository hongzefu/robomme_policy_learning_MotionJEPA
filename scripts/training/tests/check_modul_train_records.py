...
def validate(root, steps, batch_size, expected_devices="4,5"):
    ...
    require(meta["start_head"] == meta["source_head"] and len(meta["start_head"]) == 40, "源码锚点不符")
    ...
    require(int(flag(meta["argv"], "--batch-size")) == batch_size == meta["batch_size"], "实际 batch 不符")
    workers = int(flag(meta["argv"], "--num-workers"))
    ...
    if meta["history_config"] == "perceptual-framesamp-modul-32frame-8x8.yaml":
        expected = hashlib.sha256(b"bool"+str((batch_size,2048)).encode()+bytes([1])*(batch_size*2048)).hexdigest()
        require(all(row["per_key"]["['static_mask']"] == expected for row in batches),"真实训练batch并非全满历史")
        print(f"MASK_FULL=PASS batches={steps} mask_sum={batch_size*2048}")
