...
#       bash scripts/training/legacy-eval/eval_all_shards.remote.sh
...
while read -r SHARD GPU EXTRA; do
  ...
  CMD="set -o pipefail; ${ENVS} bash scripts/training/legacy-eval/eval_shard.remote.sh 2>&1 | tee -a ${LOG}; sleep 2"
