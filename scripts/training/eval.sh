...
if [ $? != 0 ]; then
    ...
    tmux send-keys -t $session_name:serve_policy "CUDA_VISIBLE_DEVICES=$GPU_ID_server uv run scripts/training/serve_policy.py --seed=$SEED  --port=$PORT policy:checkpoint --policy.dir=v1-store/train-runs/$CONFIG_TYPE/$MODEL_TYPE/$CKPT_ID --policy.config=$CONFIG_TYPE" Enter
