#!/usr/bin/env bash
# 正式评估单分片驱动（留档 docs/training-doc/eval-awsprod40k-b128-motion/）：起 policy server（uv venv；开启态由
...
#   2. 按 episode 区间分片：eval.py 的 --args.episode_start / --args.max_episodes。
# 用法：TASK=ButtonUnmask K=0 GPU=0 PORT=8031 [EP_COUNT=25] [EP_START=K*EP_COUNT] [RUN_NAME=awsprod40k-b128-motion]
#       [CKPT_ID=39999] [CKPT_DIR=…] [SEED=42] [POLICY_MEM_FRACTION=0.4] [ROBOMME_PY=…] bash scripts/training/prod/eval_shard.sh
#   结果   v1-store/evaluation/<RUN_NAME>-<TASK>-<K>/ckpt<CKPT_ID>/seed<SEED>/{progress.json,log.json,videos/}
#   server v1-store/logs/ev40k-<TASK>-<K>.server.log（含 TIMING add_buffer_ms / infer_ms 行）
#   本脚本 stdout 由 eval_all_shards.sh 经 tee 落 v1-store/logs/ev40k-<TASK>-<K>.log，结束写 EXIT_CODE=。
...
SERVER_LOG="${LOGS_DIR}/ev40k-${SHARD}.server.log"
[[ -d "${CKPT_DIR}/params" ]] || { echo "错误: checkpoint 缺 params: ${CKPT_DIR}" >&2; exit 1; }
[[ -f "$(dirname "${CKPT_DIR}")/history_config.resolved.yaml" ]] || { echo "错误: run 根缺 history_config.resolved.yaml: $(dirname "${CKPT_DIR}")" >&2; exit 1; }
...
echo "=== EVAL_SHARD shard=${SHARD} HEAD=$(git -C "${REPO_ROOT}" rev-parse HEAD) ckpt=${CKPT_DIR} gpu=${GPU} port=${PORT} seed=${SEED} episodes=[${EP_START},$((EP_START + EP_COUNT))) policy_name=${POLICY_NAME} mem_fraction=${POLICY_MEM_FRACTION} start=$(date '+%F %T') ==="
cd "${REPO_ROOT}"
...
( cd examples/robomme && CUDA_VISIBLE_DEVICES="${GPU}" PYTHONUNBUFFERED=1 "${ROBOMME_PY}" eval.py --args.port="${PORT}" --args.model_seed="${SEED}" \
    --args.policy_name="${POLICY_NAME}" --args.model_ckpt_id="${CKPT_ID}" --args.only_tasks="${TASK}" \
    --args.episode_start="${EP_START}" --args.max_episodes="${EP_COUNT}" --args.save_dir="${V1_STORE}/evaluation" ) 2>&1 || RC=$?
